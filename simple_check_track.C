...
void draw_track(Int_t n_rand)
{
    ...
    for (Int_t i = 0; i < *nhTpc; i++) {
        Int_t pad_id = padHelper::getPadID((*layerTpc)[i], (*rowTpc)[i]);

        // Skip noisy pads
        Bool_t is_found = std::binary_search(padHelper::noisy_pad.begin(), padHelper::noisy_pad.end(), pad_id);
        if (is_found) continue;

        // Fill ADC values
        h_tpc_adc2d->SetBinContent(pad_id+1, (*deTpc)[i]);
    }
